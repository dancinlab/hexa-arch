// ChemVerifyProducer — θ-2 engine tool for `chem + verify`
// (D74 cohort, scan-only consumer for anima-physics bridge records).
//
// Producer (substrate side) = anima-physics
// Langevin double-well + thermodynamic consciousness engine via
// `anima/anima-physics/tool/demiurge_chem_bridge.py`. Anima drops records
// at exports/chem/verify/<UTC>Z/anima_chem_*.json; demiurge witnesses.
//
// D17 / g_stdlib_ownership: anima-physics owns producer, demiurge consumer-only.
// HONEST (g3): GATE_OPEN / absorbed=false ALWAYS — Langevin double-well is
// consciousness-analog NOT real chemistry; no real molecular species —
// generic U(x)=(x²-1)² potential; chem oracle parity TODO.

import Foundation

public struct ChemVerifyResult: Sendable {
    public let ok: Bool
...
public enum ChemVerifyProducer {
    public static let recordsRoot: URL =
        RecordLoader.exportsRoot.appendingPathComponent(
            "chem/verify", isDirectory: true)

    public static func runVerify() -> ChemVerifyResult {
        let fm = FileManager.default
        var lines: [String] = []
        lines.append("[chem+verify] scanning \(recordsRoot.path)")

        guard fm.fileExists(atPath: recordsRoot.path) else {
            lines.append("⏳ engine tool gap — exports/chem/verify/ 미존재. "
                + "anima-physics bridge "
                + "(anima-physics/tool/demiurge_chem_bridge.py) "
                + "가 record 를 dropping 했어야 함 (g3 — silent success 금지).")
            return ChemVerifyResult(ok: false, lines: lines, newRecordID: nil)
        }
            ...
            for stamp in stamps where stamp.hasDirectoryPath {
                if let kids = try? fm.contentsOfDirectory(at: stamp,
                    ...
                    for k in kids where k.pathExtension == "json" {
                        ...
                        if name.hasPrefix("anima_") || name.hasPrefix("chem_verify_") {
        ...
        if recordPaths.isEmpty {
            lines.append("⏳ engine tool gap — exports/chem/verify/<UTC>Z/ 안에 "
                + "anima_*.json / chem_verify_*.json record 0건. anima-physics "
        ...
        lines.append("producer = \(producer) (anima-physics owns substrate; "
            + "demiurge witnesses)")
        lines.append("⏳ \(verdictGate) · absorbed=\(absorbed) — chem producer "
        ...
        return ChemVerifyResult(ok: true, lines: lines, newRecordID: recordId)
    }
}
