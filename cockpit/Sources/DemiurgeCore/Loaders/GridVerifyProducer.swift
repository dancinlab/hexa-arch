// GridVerifyProducer — θ-2 engine tool for `grid + verify`
// (D74 cohort, scan-only consumer for anima-physics bridge records).
//
// Producer (substrate side) = anima-physics Kuramoto power-grid analog via
// `anima/anima-physics/tool/demiurge_grid_bridge.py`. Anima drops records
// at exports/grid/verify/<UTC>Z/anima_grid_*.json; demiurge witnesses.
//
// Architectural distinction from GridStructureProducer
// (`grid + structure` → NetworkX IEEE 14-bus topology, cohort D57):
//   • GridStructureProducer SPAWNS networkx for graph metrics.
//   • GridVerifyProducer SCANS exports/grid/verify/ for anima bridge records.
//
// D17 / g_stdlib_ownership: anima-physics owns producer, demiurge consumer-only.
// HONEST (g3): GATE_OPEN / absorbed=false ALWAYS — Kuramoto N=8 is
// intersubjective oscillator analog NOT real PMU/SCADA;
// Filatrella 2008 Kuramoto-grid mapping = qualitative analog,
// not IEEE bus equivalent; grid oracle parity TODO.

import Foundation

public struct GridVerifyResult: Sendable {
    public let ok: Bool
...
public enum GridVerifyProducer {
    public static let recordsRoot: URL =
        RecordLoader.exportsRoot.appendingPathComponent(
            "grid/verify", isDirectory: true)

    public static func runVerify() -> GridVerifyResult {
        let fm = FileManager.default
        var lines: [String] = []
        lines.append("[grid+verify] scanning \(recordsRoot.path)")

        guard fm.fileExists(atPath: recordsRoot.path) else {
            lines.append("⏳ engine tool gap — exports/grid/verify/ 미존재. "
                + "anima-physics bridge "
                + "(anima-physics/tool/demiurge_grid_bridge.py) "
                + "가 record 를 dropping 했어야 함 (g3 — silent success 금지).")
            return GridVerifyResult(ok: false, lines: lines, newRecordID: nil)
        }
            ...
            for stamp in stamps where stamp.hasDirectoryPath {
                if let kids = try? fm.contentsOfDirectory(at: stamp,
                    ...
                    for k in kids where k.pathExtension == "json" {
                        ...
                        if name.hasPrefix("anima_") || name.hasPrefix("grid_verify_") {
        ...
        if recordPaths.isEmpty {
            lines.append("⏳ engine tool gap — exports/grid/verify/<UTC>Z/ 안에 "
                + "anima_*.json / grid_verify_*.json record 0건. anima-physics "
        ...
        lines.append("producer = \(producer) (anima-physics owns substrate; "
            + "demiurge witnesses)")
        lines.append("⏳ \(verdictGate) · absorbed=\(absorbed) — grid producer "
        ...
        return GridVerifyResult(ok: true, lines: lines, newRecordID: recordId)
    }
}
