...
- [ ] M18 외부 배포 — notarized `.app` + standalone CLI + hexa hx 의존성 · 실행 도구 ✅ `cockpit/scripts/release.hexa`(hexa-native · `hexa run` 검증: build→icon→bundle→ad-hoc sign→zip) + runbook `M18_deploy.md` · 자체서명 실증 · **코드 가능 부분 전부 ✅** · 잔여 = Developer ID + notarize (Apple cert 의존 · 코드 외)
