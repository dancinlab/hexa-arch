...
- [ ] M18 외부 배포 — notarized `.app` + standalone CLI + hexa hx 의존성 선언 · 골격 ✅ `CLI+COCKPIT/M18_deploy.md` runbook + 자체서명 경로 end-to-end 검증(release build→번들→ad-hoc codesign `Signature=adhoc`→서명 CLI 동작) · 잔여: 실행 스크립트(release.hexa/Swift) + Developer ID + notarize(Apple cert)
